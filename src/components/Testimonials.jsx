...
export default function Testimonials() {
  return (
    <section id="testimonials" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div>
          ...
          <h2 className="font-display text-3xl sm:text-4xl text-ink font-bold leading-tight">
            Rated 4.9/5 By Active Cricketers
          </h2>
        </div>

        <div className="flex items-center gap-3 bg-white px-4 py-2.5 rounded-2xl border border-ink/10 shadow-sm self-start md:self-auto">
          <div className="flex text-amber-500 text-sm">
            {'★'.repeat(5)}
          </div>
          <span className="text-xs font-bold text-ink">4.9 / 5.0</span>
          <span className="text-xs text-ink/50">(850+ Verified Reviews)</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {reviews.map((rev) => (
          <div
            key={rev.id}
            className="bg-white rounded-2xl p-7 border border-ink/10 shadow-sm flex flex-col justify-between hover:shadow-lg transition-all"
          >
            <div>
              ...
              {/* Quote */}
              <p className="text-xs sm:text-sm text-ink/80 leading-relaxed italic mb-6">
                "{rev.quote}"
              </p>
            </div>
            ...
            <div className="pt-4 border-t border-ink/5">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="font-display text-sm font-bold text-ink">{rev.name}</h4>
                  <p className="text-[11px] text-ink/60">{rev.role} • {rev.city}</p>
                </div>
